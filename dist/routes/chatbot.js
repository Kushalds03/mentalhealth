...
function generateLocalResponse(message) {
    ...
    if (has(["panic", "panic attack", "heart racing"]))
        return pick(panic);
    if (has(["anxiety", "anxious", "worry", "worried"]))
        return pick(anxiety);
    if (has(["depress", "sad", "down", "empty"]))
        return pick(lowMood);
    if (has(["stress", "overwhelmed", "overwhelm", "burnout"]))
        return pick(stress);
    if (has(["sleep", "insomnia", "can't sleep", "cant sleep"]))
        return pick(sleep);
    if (has(["angry", "anger", "rage"]))
        return pick(anger);
    if (has(["partner", "relationship", "breakup", "friend", "family"]))
        return pick(relationship);
    if (has(["work", "office", "deadline", "study", "exam", "college", "school"]))
        return pick(workStudy);
    if (has(["fever", "headache", "pain", "sick", "ill", "nausea"]))
...
router.post('/send', [
    auth_1.requireUser,
    (0, express_validator_1.body)('message').trim().notEmpty().withMessage('Message cannot be empty.'),
    (0, express_validator_1.body)('sessionId').optional().isString().withMessage('Session ID must be a string.'),
], async (req, res, next) => {
    var _a, _b;
    try {
        ...
        const userMessage = new ChatMessage_1.default({
        ...
        const aiMessage = new ChatMessage_1.default({
...
router.get('/history', auth_1.requireUser, async (req, res, next) => {
    var _a;
    try {
        ...
        const total = await ChatMessage_1.default.countDocuments(filter);
        return res.json({
...
router.get('/sessions', auth_1.requireUser, async (req, res, next) => {
    var _a;
    try {
        const sessions = await ChatMessage_1.default.aggregate([
            { $match: { user: (_a = req.user) === null || _a === void 0 ? void 0 : _a._id } },
